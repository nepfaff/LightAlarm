...
void AlarmMode::changeExistingAlarmStatus(bool newStatus)
{
  ...
  else
  {
    ...
    else if (input && (int)input - 48 <= currentAlarmQuantity)
    {
      ...
      //disable light that was enabled due to an alarm ringing soon (needs to be done before setting new status)
...
{
  //don't need to continue if not even enabled
  if (!alarm.getStatus()) {
    return false;
  }

  //calculate the time when the light would be activated due to the alarm
  int activateHour{}, activateMinute{};
  if (alarm.getMinute() >= timeToActivateLightMin) {
    activateHour = alarm.getHour();
    activateMinute = alarm.getMinute() - timeToActivateLightMin;
  } else {
    activateHour = alarm.getHour() - 1;
    activateMinute = alarm.getMinute() + 60 - timeToActivateLightMin;
  }
